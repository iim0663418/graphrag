...
const SearchInterface = () => {
  ...
  const handleSearch = (e, q = query) => {
    ...
    setTimeout(() => {
      setResults([
        { id: 1, title: 'GraphRAG 核心架構解析', snippet: '系統採用 D3.js 進行圖譜視覺化，結合 React 18 的並發模式與地端向量庫，實現高效的語義對齊與跨文檔查詢...', category: '技術規範', source: '架構設計方案.pdf', score: 0.98 },
        { id: 2, title: '2024 年度知識管理洞察', snippet: '地端知識管理的核心在於隱私保護與深度關聯。透過 GraphRAG 模式，我們能更有效地組織非結構化數據，並從中發現隱性聯繫...', category: '市場報告', source: '2024_Q4_研究.txt', score: 0.82 }
      ]);
    ...
    <div className="max-w-4xl mx-auto space-y-12 animate-in fade-in duration-700">
      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-wrap gap-2 items-center">
          ...
          <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest mr-3">熱門查詢建議</span>
          {suggestions.map((s, i) => (
            <button 
              key={i} 
              onClick={() => { setQuery(s); handleSearch(null, s); }}
              className="text-[10px] font-black bg-white border border-slate-200 px-4 py-2 rounded-full text-slate-500 hover:border-blue-400 hover:text-blue-600 transition-all shadow-sm tracking-wide"
            >
              {s}
            </button>
          ))}
...
const FileUploadZone = () => {
  ...
  return (
    ...
    >
      ...
      <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.4em]">拖放檔案至此處執行離線分析 • 限制 20MB • 支援 .TXT, .CSV</p>
    </div>
  );
};
...
const KnowledgeTopology = () => {
  ...
  useEffect(() => {
    ...
    const data = {
      nodes: [
        { id: 'GraphRAG', group: 1, val: 34 }, { id: 'VectorDB', group: 4, val: 24 },
        { id: 'LLM Engine', group: 4, val: 26 }, { id: 'React 18', group: 2, val: 20 },
        { id: 'D3.js', group: 2, val: 18 }, { id: 'Zustand', group: 3, val: 16 },
        { id: 'Tailwind', group: 2, val: 14 }
      ],
      links: [
        { source: 'GraphRAG', target: 'VectorDB' }, { source: 'GraphRAG', target: 'LLM Engine' },
        { source: 'GraphRAG', target: 'React 18' }, { source: 'React 18', target: 'D3.js' },
        { source: 'React 18', target: 'Zustand' }, { source: 'React 18', target: 'Tailwind' }
      ]
    };
    ...
    <div className="flex h-[660px] space-x-12">
      <div ref={containerRef} className="flex-1 bg-white rounded-[60px] border border-slate-100 shadow-2xl relative overflow-hidden">
        ...
        <div className="absolute top-10 left-10 bg-slate-900 text-white px-6 py-2.5 rounded-[20px] text-[10px] font-black uppercase tracking-[0.25em] shadow-2xl flex items-center border border-slate-700">
          <Activity size={14} className="mr-3 text-emerald-400 animate-pulse" /> Knowledge Topology Network
        </div>
      </div>
